...
use Modules\Vendor\Transformers\Dashboard\BranchesSelectResource;
...
class VendorController extends Controller
{
    ...
    public function getAllActiveVendors()
    {
        ...
        return Response()->json([true, 'data' => $vendors]);
    }
    ...
    public function store(VendorRequest/*Request*/ $request)
    {
        try {
...
//            if (!isset(config('setting.other')['is_multi_vendors']) || config('setting.other')['is_multi_vendors'] == 1) {
    ...
    public function getBranchesByRestaurant(Request $request)
    {
        $branches = $this->vendor->getAllByParentId($request->restaurant_id);
        return Response()->json(['status' => true, 'data' => BranchesSelectResource::collection($branches)]);
    }
