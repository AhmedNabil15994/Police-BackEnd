...
class AreaController extends WebServiceController
{
    ...
    public function citiesWithStates(Request $request)
    {
        $cities = $this->area->citiesWithStates($request);
        $result = $cities ? CityResource::collection($cities) : [];
        return $this->response($result);
    }


    public function statesWithBranches(Request $request)
    {
        $states = $this->area->statesWithBranches($request);
        dd($states->toArray());
       /* $result = $cities ? CityResource::collection($cities) : [];
        return $this->response($result);*/
    }
