...
class ProductRequest extends FormRequest
{
        ...
        switch ($this->getMethod()) {
            ...
            case 'POST':
                ...
                if ($this->manage_qty == 'limited')
                    $result['qty'] = 'required|integer|min:1';
                ...
                if ($this->customize_ordering_time) {
                    $result['days_status'] = 'required|array|min:1';
                }
            ...
            case 'PUT':
                ...
                if ($this->manage_qty == 'limited')
                    $res['qty'] = 'required|integer|min:1';
                ...
                if (auth()->user()->can('edit_products_price')) {
                    $res['price'] = 'required|numeric|min:0';
                }
                ...
                if ($this->customize_ordering_time) {
                    $res['days_status'] = 'required|array|min:1';
                }
    ...
    {
        $v = [
            'restaurants.required' => __('catalog::dashboard.products.validation.restaurants.required'),
            'restaurants.array' => __('catalog::dashboard.products.validation.restaurants.array'),
            'restaurants.min' => __('catalog::dashboard.products.validation.restaurants.min'),
            ...
            'qty.min' => __('catalog::dashboard.products.validation.qty.min'),

            'price.numeric' => __('catalog::dashboard.products.validation.price.numeric'),
            'price.min' => __('catalog::dashboard.products.validation.price.min'),
            ...
            'days_status.required' => __('apps::dashboard.working_times.validations.days_status.required'),
            'days_status.array' => __('apps::dashboard.working_times.validations.days_status.array'),
            'days_status.min' => __('apps::dashboard.working_times.validations.days_status.min'),
        ];
            ...
            foreach ($this->days_status as $k => $dayCode) {
                if (array_key_exists($dayCode, $this->is_full_day)) {
                    if ($this->is_full_day[$dayCode] == '0') {
                        ...
                        $duplicatedMsg = __('apps::dashboard.working_times.form.day');
                        $duplicatedMsg .= ' " ' . __('apps::dashboard.working_times.days.' . $dayCode) . ' " ';
                        $duplicatedMsg .= __('apps::dashboard.working_times.validations.contain_duplicated_values');
                        $v['availability.duplicated_time.' . $dayCode . '.required'] = $duplicatedMsg;

                        foreach ($this->availability['time_from'][$dayCode] as $key => $time) {

                            if (strtotime($this->availability['time_to'][$dayCode][$key]) < strtotime($time)) {
                                $requiredMsg = __('apps::dashboard.working_times.form.time');
                                $requiredMsg .= ' " ' . $time . ' " ';
                                $requiredMsg .= __('apps::dashboard.working_times.form.for_day');
                                $requiredMsg .= ' " ' . __('apps::dashboard.working_times.days.' . $dayCode) . ' " ';
                                $requiredMsg .= " " . __('apps::dashboard.working_times.validations.greater_than') . " ";
                                $requiredMsg .= " " . __('apps::dashboard.working_times.form.time') . " ";
