        ...
        <div class="page-content">
            ...
            <div class="row">
                <form id="addOnsForm" class="form-horizontal form-row-seperated" method="post"
                      ...
                      action="{{route('vendor.products.store_add_ons', $product->id)}}">
                    @csrf
                    @method('POST')
                    <div class="col-md-12">
                        ...
                        <div class="col-md-9">
                            <div class="tab-content">
                                ...
                                <h4>{{ __('catalog::vendor.products.form.add_ons.product') }}
                                    : {{ $product->translate(locale())->title }}</h4>
                                <hr>
                                    ...
                                    <div class="col-md-10">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-9">
                                                <div class="mt-radio-inline">
                                                    <label
                                                        class="mt-radio mt-radio-outline"> {{ __('catalog::vendor.products.form.add_ons.single') }}
                                        ...
                                        <table class="table table-striped table-bordered table-hover">
                                            ...
                                            <tr>
                                                ...
                                                <th>{{__('catalog::vendor.products.form.add_ons.image')}}</th>
                                            ...
                                            <tr id="optionRow-0">
                                                ...
                                                <td>
                                                    <input type="number" step="0.01" class="form-control optionPrice"
                                                           name="price[]"
                                                           autocomplete="off">
                                                </td>
                                                <td>
                                                    <input type="file" class="form-control optionImage" name="image[]"
                                                           accept="image/x-png,image/gif,image/jpeg,image/svg"
                                                           onchange="readURL(this, 'imgUploadPreview-0', 'single');">
                                                    <img id='imgUploadPreview-0'
                                                         style="display: none; width: 70px; height: 50px;"
                                                         src="#" class="img-preview img-thumbnail" alt="image preview"/>
                                                </td>
                                        ...
                                        <div id="optionsCountRow" class="row" style="display: none;">
                                            <div class="col-md-12">
                                                <div
                                                    class="pull-left">{{ __('catalog::vendor.products.form.add_ons.customer_can_select_exactly') }}
                                        ...
                                        <a href="javascript:;" style="margin-left: 10px; margin-right: 10px;"
                                           ...
                                           onclick="clearDefaults()">
                                            <i class="fa fa-remove"></i> {{__('catalog::vendor.products.form.add_ons.clear_defaults')}}
                                        </a>
                                        <a id="resetForm" href="javascript:;"
                                           style="display: none; margin-left: 10px; margin-right: 10px;"
                                           class="pull-right btn btn-primary"
                                           onclick="defaultMode()">
                                            <i class="fa fa-refresh"></i> {{__('catalog::vendor.products.form.add_ons.reset_form')}}
                                        </a>
...
@section('scripts')
    ...
    <script>
        ...
        function addMoreOptions() {
           ...
           name="option[${rowCount}][{{$code}}]" placeholder="{{__('catalog::vendor.products.form.add_ons.option')}} - {{ $code }}" autocomplete="off">
                                @endforeach
            </td>
            <td><input type="number" step="0.01" class="form-control optionPrice" name="price[]" autocomplete="off"></td>
            <td>
                <input type="file" class="form-control optionImage" name="image[]" accept="image/x-png,image/gif,image/jpeg,image/svg" onchange="readURL(this, 'imgUploadPreview-${rowCount}', 'single');">
                <img id='imgUploadPreview-${rowCount}'
                 style="display: none; width: 70px; height: 50px;"
                 src="#" class="img-preview img-thumbnail" alt="image preview"/>
            </td>
        ...
        function deleteOptions(index, rowId, flag = '') {
            ...
            if (rowCountsArray.length > 1) {
                ...
                if (flag === 'db') {
                    ...
                    var r = confirm("{{ __('catalog::vendor.products.form.add_ons.confirm_msg') }}");
                    if (r == true) {

                        $.ajax({
                            url: "{{route('vendor.products.delete_add_ons_option')}}?id=" + rowId,
                            type: 'get',
                            dataType: 'JSON',
                            contentType: false,
                            cache: false,
                ...
                alert("{{__('catalog::vendor.products.form.add_ons.at_least_one_field')}}");
                return false;
            }
        }
        ...
        $('#addOnsForm').on('submit', function (e) {
            ...
            if (optionsCount && optionsCount !== '') {
                ...
                if (optionsCount > rows.length) {
                    alert("{{__('catalog::vendor.products.form.add_ons.options_count_greater_than_rows')}}");
                    return;
                }
            }
        ...
        function deleteMasterOptions(rowId, flag = '') {
            ...
            if (flag === 'db') {
                ...
                if (r == true) {
                    ...
                    $.ajax({
                        url: "{{route('vendor.products.delete_add_ons')}}?id=" + rowId,
                        type: 'get',
                        dataType: 'JSON',
                        contentType: false,
                        cache: false,
        ...
        function appendNewOptionRow(data) {
            if (data.type === 'single') {
                addOnsType = "{{ __('catalog::vendor.products.form.add_ons.single') }}";
            } else {
                addOnsType = "{{ __('catalog::vendor.products.form.add_ons.multiple') }}";
            }
            ...
            var newRow = `<tr id="addOnsRow-${data.id}">
                                            ...
                                            <a href="javascript:void(0)" class="btn btn-warning"
                                               onclick="openAddOnsDetails('row', ${data.id})" title="{{__('catalog::vendor.products.form.add_ons.show')}}"><i class="fa fa-eye"></i></a>
        ...
        function openAddOnsDetails(flag = '', rowId, data = {}) {
                          ...
                          <div class="modal-dialog" role="document">
                             <div class="modal-content">
                                  <div class="modal-header">
                                        <h5 class="modal-title" id="modalLabel-${data.id}">{{ __('catalog::vendor.products.form.tabs.add_ons') }} : {{ $product->translate(locale())->title }}</h5>
                                  ...
                                  <div class="modal-body">
                                    ...
                                    <div style='margin-bottom: 7px;'>{{__('catalog::vendor.products.form.add_ons.add_ons_name')}} : <b>${data.name}</b></div>
                                    <div style='margin-bottom: 7px;'>{{__('catalog::vendor.products.form.add_ons.type')}} : <b>${addOnsType}</b></div>
                                    <div style='margin-bottom: 7px;'>{{__('catalog::vendor.products.form.add_ons.options_count')}} : <b>${data.options_count == null ? '---' : data.options_count}</b></div>
                                    <div style='margin-bottom: 7px;'>{{__('catalog::vendor.products.form.add_ons.created_at')}} : <b>${data.created_at}</b></div>
                                    <br>
                                    <table class="table table-striped table-bordered table-hover">
                                        <thead>
                                        <tr>
            ...
            for (let value of data.add_on_options) {
                ...
                <tr>
                   <td>${value.id}</td>`;
                if (value.image != null) {
                    var imgUrl = '{{ url('') }}' + '/' + value.image;
                    modal += `<td><img src="${imgUrl}" style="width: 70px; height: 50px;"></td>`;
                } else {
                    modal += `<td>---</td>`;
                }
                modal += `<td>${value.option}</td>
        ...
        function editMasterOptions(flag = '', rowId, data = {}) {
            ...
            resetForm.show();
            var pTitle = "# {{ __('catalog::vendor.products.form.tabs.edit_add_ons') }}" + " : " + data.name;
            ...
            data.add_on_options.forEach(function (item, i) {
                ...
                item.translations.forEach(function (element, inx) {

                    newRow += `<input type="text" class="form-control add_ons_option"
                                name="option[${i}][${element.locale}]"
                                value="${element.option}"
                                 placeholder="{{__('catalog::vendor.products.form.add_ons.option')}} - ${element.locale}" autocomplete="off">`;
                });

                newRow += `</td>
                        <td><input type="number" step="0.01" class="form-control optionPrice" name="price[]" autocomplete="off" value="${parseFloat(item.price).toFixed(2)}"></td>
                        <td>
                            <input type="file" class="form-control optionImage" name="image[]" accept="image/x-png,image/gif,image/jpeg,image/svg" onchange="readURL(this, 'imgUploadPreview-${i}', 'single');">`;

                if (item.image != null) {
                    let imgUrl = '{{ url('') }}' + '/' + item.image;
                    newRow += `<img id='imgUploadPreview-${i}'
                         style="width: 70px; height: 50px;"
                         src="${imgUrl}" class="img-preview img-thumbnail" alt="image preview"/>`;
                } else {
                    newRow += `<img id='imgUploadPreview-${i}'
                         style="display: none; width: 70px; height: 50px;"
                         src="#" class="img-preview img-thumbnail" alt="image preview"/>`;
        ...
        function defaultMode(flag = '') {
            ...
            {{-- var pTitle = "# {{ __('catalog::vendor.products.form.tabs.add_ons') }}";
            pageTitle.text(pTitle);--}}
            ...
            optionsCountRow.hide();
            $('#optionRow-0').nextAll().remove();
            $("#optionRow-0 td:nth-child(5)").html('<button type="button" class="btn btn-danger" onclick="deleteOptions(0, 0, \'row\')">x</button>');
            $('.optionPrice').val('');
        ...
        function assignAddOnsName(flag = '', data = {}) {
            ...
            if (flag === 'reset') {
                ...
                @foreach (config('translatable.locales') as $k => $code)
                $("input[name='option_name[" + data.translations["{{$k}}"].locale + "]']").val(data.translations["{{$k}}"].name);
                @endforeach
            }
