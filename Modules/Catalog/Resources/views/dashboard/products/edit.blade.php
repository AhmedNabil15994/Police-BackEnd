...
@section('css')
    <style>
        ...
        /**************************** Start - Customize Ordering Time Checkbox *************************/
        .is_full_day {
            margin-left: 15px;
            margin-right: 15px;
        }
        ...
        /**************************** End - Customize Ordering Time Checkbox *************************/
                    ...
                    <div class="col-md-12">
                                    ...
                                    <div id="collapse_2_1" class="panel-collapse in">
                                        <div class="panel-body">
                                            <ul class="nav nav-pills nav-stacked">
                                                ...
                                                <li class="" id="workingTimesTab"
                                                    style="{{ $product->workingTimes()->count() > 0 ? '':'display: none;' }}">
                                                    <a href="#workingTimes" data-toggle="tab">
                                                        {{ __('catalog::dashboard.products.form.tabs.ordering_times') }}
                                                    </a>
                                                </li>
                        ...
                        <div class="col-md-9">
                            <div class="tab-content">
                                    ...
                                    <div class="tab-content">
                                        ...
                                        <div class="col-md-10">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <label class="col-md-2">
                                                    {{__('catalog::dashboard.products.form.restaurants')}}
                                                </label>
                                                <div class="col-md-9">
                                                    <select id="restaurantSelect" name="restaurants[]"
                                                            class="form-control select2" multiple="">
                                                        <option
                                                            value="">{{__('apps::dashboard.general.select_option')}}</option>
                                                        @foreach ($restaurants as $item)
                                                            <option value="{{ $item['id'] }}"
                                                                {{ $product->productProviders->contains($item->id) ? 'selected': '' }}>
                                            ...
                                            <div id="restaurantLoader" class="text-center m-3"
                                                 style="display: none; color: #494949;">
                                                <b>{{ __('apps::dashboard.general.loader') }}</b>
                                            </div>
                                            <div id="restaurantBranches">
                                                @foreach ($restaurants as $item)
                                                    @if($product->productProviders->contains($item->id))
                                                        <div class="form-group" id="branchArea-{{ $item->id }}">
                                                            <div class="col-md-2"></div>
                                                            <div class="col-md-9">
                                                                <label>
                                                                    {{__('catalog::dashboard.products.form.branches')}}
                                                                    : # {{ $item->translate(locale())->title }}
                                                                </label>
                                                                <select id="branchesSelect"
                                                                        name="branches[{{ $item->id }}][]"
                                                                        class="form-control select2"
                                                                        multiple="">
                                                                    @foreach (getAllActiveBranchesByRestaurantId($item->id) as $element)
                                                                        <option
                                                                            value="{{ $element['id'] }}" {{ $product->productVendors->contains($element->id) ? 'selected': '' }}>
                                                                            {{ $element->translate(locale())->title }}
                                                                        </option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                            ...
                                            <div class="form-group">
                                                <label class="col-md-2">
                                                    {{__('catalog::dashboard.products.form.customize_ordering_time')}}
                                                </label>
                                                <div class="col-md-9">
                                                    <input type="checkbox" class="make-switch"
                                                           id="customizeOrderingTimeCheckbox"
                                                           data-size="small"
                                                           name="customize_ordering_time" {{($product->workingTimes()->count() > 0) ? ' checked="" ' : ''}}>
                                                    <div class="help-block"></div>
                                                </div>
                                            </div>
                                ...
                                <div class="tab-pane fade in" id="workingTimes">
                                    @include('catalog::dashboard.products.working_times._edit_times')
                                </div>
...
@section('scripts')
    ...
    <script>
        var rowCountsArray = [];
        var workingTimesRowCountsArray = [];
        ...
        $(document).ready(function () {
            ...
            $('#customizeOrderingTimeCheckbox').on('switchChange.bootstrapSwitch', function (e) {
                $('#workingTimesTab').toggle(e.target.checked);
            });
        ...
        function addMoreDayTimes(e, dayCode) {
            ...
            var rowCount = Math.floor(Math.random() * 9000000000) + 1000000000;
            workingTimesRowCountsArray.push(rowCount);
        ...
        function removeDayTimes(dayCode, index, flag = '') {
            ...
            {{-- if (workingTimesRowCountsArray.length > 1) { --}}

            if (flag === 'row') {
                $('#rowId-' + dayCode + '-' + index).remove();
                const i = workingTimesRowCountsArray.indexOf(index);
                if (i > -1) {
                    workingTimesRowCountsArray.splice(i, 1);
                }
            }
