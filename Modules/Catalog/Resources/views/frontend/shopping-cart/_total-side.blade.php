    ...
    <div class="minicart-content-wrapper">
                                ...
                                <div class="media-body">
                                            <span class="product-name">
                                                ...
                                                @if($item->attributes->product_type == 'product')
                                                    <a
                                                        href="{{ route('frontend.products.index', [$item->attributes->product->translate(locale())->slug]) }}">
                                                    {{ $item->attributes->product->translate(locale())->title }}
                                                </a>
                                                @else
                                                    <a
                                                        href="{{ route('frontend.products.index', [$item->attributes->product->product->translate(locale())->slug, generateVariantProductData($item->attributes->product->product, $item->attributes->product->id, $item->attributes->selectedOptionsValue)['slug']]) }}">
        ...
        <div class="d-flex mb-20 align-items-center">
            ...
            <span id="cartSubTotal"
                  class="d-inline-block left-side">{{ number_format(getCartSubTotal(), 2) }} {{ __('apps::frontend.master.kwd') }}</span>
        </div>
                ...
                <div class="d-flex mb-20 align-items-center mb-3">
                    ...
                    <span class="d-inline-block left-side">
                        <span id="totalDeliveryPrice">
                            {{ number_format(Cart::getCondition('company_delivery_fees')->getValue(), 2) }}
                        </span>
        ...
        <div id="couponContainer">
            @if(Cart::getCondition('coupon_discount') != null)
                <div class="d-flex mb-20 align-items-center">
                    ...
                    <span
                        class="d-inline-block left-side">{{ number_format(abs(Cart::getCondition('coupon_discount')->getValue())) }} {{ __('apps::frontend.master.kwd') }}</span>
                </div>
            @endif
        </div>
        ...
        <div class="minicart-footer">
            ...
            <div class="subtotal d-flex text-center justify-content-center mb-20">
                ...
                <span id="cartTotalAmount"
                      class="price">{{ number_format(getCartTotal(), 2) }} {{ __('apps::frontend.master.kwd') }}</span>
            </div>
            ...
            @else
                <div class="actions">
                    <a class="btn btn-checkout btn-them btn-block"
