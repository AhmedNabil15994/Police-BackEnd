                    ...
                    <div class="list-group" id="allStates">
                        @if(count($states) > 0)
                            @foreach ($states as $state)
                                <a href="javascript:;"
                                   id="state-{{$state->id}}"
                                   onclick="saveArea('{{ $state->id }}', '{{ $state->translate(locale())->title }}', '{{ $state->deliveryCharge->delivery_time }}', '{{ $state->deliveryCharge->delivery }}')"
                                ...
                                @if(count($states) > 0)
                                    @foreach ($states as $state)
                                        <li id="state-{{ $state->id }}" class="state"
                                            ...
                                            data-name="{{ $state->translate(locale())->title }}">{{ $state->translate(locale())->title }}</li>
                                    @endforeach
                                @endif
                            </ul>
                        </div>
