...
<script>
    ...
    pusher.subscribe('{{ config('core.config.constants.VENDOR_DASHBOARD_CHANNEL') }}').bind('{{ config('core.config.constants.VENDOR_DASHBOARD_ACTIVITY_LOG') }}', function (data) {

        $('#dataTable').DataTable().ajax.reload();
        var userId = parseInt('{{ auth()->id() }}');

        if (jQuery.inArray(userId, data.activity.ids) !== -1) {
            openActivity(data.activity);
        }

        {{--console.log('data.activity:::', data.activity, 'data.activity.ids:::', data.activity.ids, 'user_id', '{{ auth()->id() }}');
