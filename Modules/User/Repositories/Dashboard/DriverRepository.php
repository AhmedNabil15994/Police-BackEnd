...
class DriverRepository
{
    ...
    */
    public function getAllDrivers($order = 'id', $sort = 'desc')
    {
        ...
        // return $this->user->role('driver_access')->orderBy($order, $sort)->get();
    }
...
//                'company_id' => $request['company_id'] ?? null,
            ...
            if ($request->vendor_id)
                $user->branches()->sync($request->vendor_id);
...
//                'company_id' => $request['company_id'] ?? null,
    ...
    */
    ...
    {
        $query = $this->user->with('branches')->where('id', '!=', auth()->id())->whereHas('roles.perms', function ($query) {
            $query->where('name', 'driver_access');
            ...
            $query->orWhereHas('branches.translations', function ($q) use ($request) {
                $q->where('title', 'like', '%' . $request->input('search.value') . '%');
            });

            /*$query->orWhereHas('company.translations', function ($q) use ($request) {
